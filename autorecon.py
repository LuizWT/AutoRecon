...
from functions.check_and_install_tool import check_and_install_tool # Função para checar e instalar ferramentas
...
async def main_loop():
    ...
    async def _(event):
        if global_target.value:
            ...
            clear_terminal()
            logger.error(f"Você deve definir um alvo global antes de acessar este submenu.\n{Fore.CYAN}Pressione Enter para retornar...")
            return
    ...
    while True:
        ...
        if option == "9":
            clear_terminal()
            logger.info(f"Saindo do AutoRecon.")
            break
            ...
            clear_terminal()
            logger.info(f"Verificando a instalação do ProxyChains...")
            ...
            if ProxyManager.check_installed():
                ...
                if install_choice.lower() in ['s', 'y']:
                    ...
                    if ProxyManager.check_installed():
                        ...
                        logger.error(f"Falha ao instalar o ProxyChains.")
                    await session.prompt_async(f"{Fore.YELLOW}Pressione Enter para retornar ao menu principal...")
                else:
                    logger.info(f"Retornando ao menu principal...")
                    await session.prompt_async(f"{Fore.YELLOW}Pressione Enter para retornar ao menu principal...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        clear_terminal()
        logger.info(f"Saindo do AutoRecon via Ctrl+C.")